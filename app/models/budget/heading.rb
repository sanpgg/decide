class Budget
  class Heading < ActiveRecord::Base
    ...
    scope :order_by_group_name_condition, ->(sec, col_id) { where('sector = ? OR suburb_id = \'?\'', sec, col_id).includes(:group).order('budget_groups.name', 'budget_headings.name') }
    scope :order_by_group_name_no_null_description, -> { where.not(description: '').includes(:group).order('budget_groups.name', 'budget_headings.name') }
    scope :allow_custom_content, -> { where(allow_custom_content: true).order(:name) }
    #scope :sectors, -> { includes(:group).where.not(sector: '') }
    scope :sectors, -> { find_by(id: 11) }
    scope :by_budget,         ->(budget)      { where(budget: budget) }

    def name_scoped_by_group
      group.single_heading_group? ? name : "#{group.name}: #{name} (Proyecto de #{htype == 'sector' ? 'Sector' : 'Colonias'})"
    end
